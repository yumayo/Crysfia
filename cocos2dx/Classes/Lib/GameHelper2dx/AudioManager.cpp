...
namespace gameHelper2dx
{

    UserAudioManager* UserAudioManager::_instance = nullptr;
    cocos2d::Scheduler* UserAudioManager::_scheduler = nullptr;
    ...
    // 初期化
    UserAudioManager* UserAudioManager::getInstance( ) {

        if ( _instance == nullptr ) {
            _instance = new UserAudioManager( );
    ...
    // 削除する際に使用
    void UserAudioManager::deleteInstance( ) {
    ...
    // オーディオ管理ファイルを読み込む
    bool UserAudioManager::readAudioListFile( const std::string fileName ) {
    ...
    std::string UserAudioManager::getFileName( AudioType type, std::string baseName ) {
    ...
    void UserAudioManager::releaseAll( ) {
        AudioEngine::uncacheAll( );
    }

    // 毎フレーム実行
    void UserAudioManager::update( float dt ) {
    ...
    // BGMとSEの音量の初期化
    void UserAudioManager::initVolume( float bgmVolume, float seVolume ) {
        _bgmVolume = bgmVolume;
        _seVolume = seVolume;
    }

    // モバイルデバイスかどうか
    bool UserAudioManager::isMobileDevice( ) {
        auto platform = Application::getInstance( )->getTargetPlatform( );
    ...
    // BGMのPreLoad
    void UserAudioManager::preloadBgm( const std::string baseName ) {
    ...
    // BGMの再生
    int UserAudioManager::playBgm( const std::string baseName, float fadeTime /* =0*/, bool loop /* = true*/ ) {
        return playBgm( baseName, fadeTime, loop, _bgmVolume );
    }
    // BGMの再生
    int UserAudioManager::playBgm( const std::string baseName, float fadeTime, bool loop, float volume ) {
    ...
    // BGMを一時停止する
    void UserAudioManager::pauseBgm( float fadeTime /*= 0*/ ) {
    ...
    void UserAudioManager::pauseBgmEngine( ) {
        AudioEngine::pause( _bgmId );
    }

    // BGMをリジューム再生する
    void UserAudioManager::resumeBgm( float fadeTime /*=0*/ ) {
    ...
    // BGMを停止する
    void UserAudioManager::stopBgm( float fadeTime /*= 0*/, bool release /* = true */ ) {
    ...
    void UserAudioManager::stopBgmEngine( bool release /* = true */ ) {
    ...
    // BGMが再生されているかどうか
    bool UserAudioManager::isPlayingBgm( ) {
    ...
    // BGMの音量を変更する
    void UserAudioManager::setBgmVolume( float volume, bool save /* = true */ ) {
    ...
    // BGMの音量を取得する
    float UserAudioManager::getBgmVolume( ) {
        return _bgmVolume;
    }

    // BGMのキャシュを解放する
    void UserAudioManager::releaseBgm( ) {
    ...
    // 効果音のPreLoad
    void UserAudioManager::preloadSe( const std::string baseName ) {
    ...
    // 効果音を再生する
    int UserAudioManager::playSe( const std::string baseName, int chunkNo ) {
        return this->playSe( baseName, chunkNo, false, _seVolume );
    }
    // 効果音を再生する
    int UserAudioManager::playSe( const std::string baseName, int chunkNo, bool loop, float volume ) {
    ...
    // 効果音を再生する
    int UserAudioManager::playSe( const std::string baseName, bool loop, float volume ) {

        return this->playSe( baseName, -1, loop, volume );
    }
    // 効果音を再生する
    int UserAudioManager::playSe( const std::string baseName, bool loop /* = false */ ) {
        return this->playSe( baseName, loop, _seVolume );
    }

    // 効果音を停止する
    void UserAudioManager::stopSe( int soundId ) {
        AudioEngine::stop( soundId );
    }

    // 効果音の音量を変更する
    void UserAudioManager::setSeVolume( float volume ) {
        _seVolume = volume;
        //AudioEngine::setVolume(soundId, _seVolume);
    }

    // 効果音の音量を取得する
    float UserAudioManager::getSeVolume( ) {
        return _seVolume;
    }

    // 効果音のキャッシュを解放する
    void UserAudioManager::releaseSe( const std::string baseName ) {
    ...
    // AudioEngineを解放する
    void UserAudioManager::endAudioEngine( ) {
        AudioEngine::end( );
    }
