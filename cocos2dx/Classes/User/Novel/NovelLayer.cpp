...
bool NovelLayer::init( )
{
    ...
    key->onKeyPressed = [ this ] ( cocos2d::EventKeyboard::KeyCode keyCode, cocos2d::Event* keyEvent )
    {
        if ( keyCode == cocos2d::EventKeyboard::KeyCode::KEY_D )
        {
            ...
            {
                layer->removeChildByName( "LayerDebugText" );

                if ( debugMode )
                {
                    layer->addChild( LayerDebugText::create( debugData.debugData.fileName, debugData.debugData.lineNumber ) );
                }
            }
        }
    };
